...
assert "allNodes total = 226"       "$(ev "_LG.allNodes.length + ''")"  "226"
...
assert "layer 4 nodes = 35 (bugs+patterns)"  "$(ev "_LG.allNodes.filter(function(n){return n.layer===4}).length + ''")"  "35"
...
assert "meshes count = 226"         "$(ev "_LG.meshes.length + ''")"  "226"
...
assert "group bug-partial = 2"   "$(ev "_LG.allNodes.filter(function(n){return n.group==='bug-partial'}).length + ''")"  "2"
...
assert "all-labels on — all 226 labelObjs visible"   \
...
if [[ "$batch_result" == PASS:* ]]; then
  ...
  fail "batch select/deselect" "$batch_result" "PASS:226"
fi
